...
glmReadMTL(GLMmodel* model, char* name)
{
	...
	while(fscanf(file, "%s", buf) != EOF) {
		...
		case 'K':
			...
			break;
		case 'm':
			if(buf[1]=='a' && buf[2] == 'p' && buf[3]=='_' && buf[4] =='K' && buf[5]=='d'){//map_Kd
				fgets(buf, sizeof(buf),file);
				sscanf(buf, "%s %s", buf, buf);// texture fileName;
				if(model->texturename)
					free(model->texturename);
				model->texturename = _strdup_(buf);
				//   printf("texture name == %s",model->texturename);
			}
			break;
		default:
			/* eat up rest of line */
 ...
 */
GLvoid
glmDimensions(GLMmodel* model, GLfloat* dimensions, GLfloat ys[2])
{
    GLuint i;
    GLfloat maxx, minx, maxy, miny, maxz, minz;
	...
	ys[0] = maxy;
	ys[1] = miny;
	printf("dimensions: x:(%f, %f), y:(%f, %f), z:(%f, %f)",maxx, minx, maxy, miny, maxz, minz );
}
...
glmLinearTexture(GLMmodel* model)
{
    ...
    glmDimensions(model, dimensions,ys);
    scalefactor = 2.0 / 
...
glmSpheremapTexture(GLMmodel* model)
{
    ...
    for (i = 1; i <= model->numnormals; i++) {
        ...
        if(FLOAT_EQUALS_ZERO(r)/* == 0.0*/) {
            theta = 0.0;
            phi = 0.0;
        } else {
            if(FLOAT_EQUALS_ZERO(z)/* == 0.0*/)
                phi = M_PI / 2.0;
            else
                phi = acos(z / rho);
            
            if(FLOAT_EQUALS_ZERO(y)/* == 0.0*/)
                theta = M_PI / 2.0;
            else
...
glmDelete(GLMmodel* model)
{
    ...
    if(model->texturename) free(model->texturename);
