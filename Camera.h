...
typedef class Interface_VCap{
	public:
		...
		virtual void CaptureTouchSrc(char* ptr,bool istouchsrc)=0;
		virtual unsigned char *GetSrc()=0;
...
class BaseVCap:public Interface_VCap{
	public:
		...
		virtual ~BaseVCap()=0;
		virtual 	void CaptureTouchSrc(char* ptr,bool istouchsrc){};
		virtual unsigned char *GetSrc();
	protected:
...
public:
	...
	yuvImg = Mat(IMAGEHEIGHT,IMAGEWIDTH,CV_8UC3);};
...
public:
	...
	void Close(){};
	void Capture(char* ptr);
	void CaptureTouchSrc(char* ptr,bool istouchsrc);
	void CaptureFish(char* ptr){};
	void SavePic(const char* name);
	void saveOverLap(){};
