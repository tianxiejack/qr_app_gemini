...
bool PBOSender::sendDataPBO(GLEnv &env,GLuint textureId, PFN_PBOFILLBUFFER fxn, GLuint idx)
{
...
bool PBOSender::sendData(GLEnv &env,GLuint textureId, PFN_PBOFILLBUFFER fxn, GLuint idx,bool bPBO)
{
	bool Isenhdata=false;
	if(bPBO&&bUsePBO)
		Isenhdata=sendDataPBO(env,textureId, fxn, idx);
	else
	{
